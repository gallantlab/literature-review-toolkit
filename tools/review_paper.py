...
def reference_list(rows):
    """The APA-7 reference list every renderer prints: [(apa, link)] from the
    canonical `apa` strings — deduped on the text, alphabetized with accents
    folded, and terminated the way the audit gate requires (a title ending in
    ? or ! takes no period). ONE implementation, so the .docx and any HTML page
    built from the same rows.json cannot disagree."""
    seen, refs = set(), []
    for row in rows:
        apa = (row.get("apa") or "").strip()
        if apa and apa not in seen:
            seen.add(apa)
            refs.append((apa if apa.endswith((".", "?", "!")) else apa + ".", row.get("link", "")))
...
def build(content, rows, out, figure=None):
    ...
    # ---- references: APA-7 from rows.json apa, deduped + alphabetized + hanging indent
    doc.add_page_break()
    doc.add_heading(content.get("references_heading", "References"), level=1)
    refs = reference_list(rows)
...
def main():
    ...
    ap.add_argument("--content", required=True,
                    help="content.json (prose: title/author/abstract/sections/figure)")
